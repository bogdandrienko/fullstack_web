...
def new():
    ...
    student_john = Student(
        firstname=f"john {rand}",
        lastname=f"doe {rand}",
        email=f"jd {rand} @example.com",
        age=23,
        bio="Biology student",
    )
    ...
    students: list[Student] = Student.query.all()  # Student.query.filter_by(firstname='Sammy').all()
    print(type(students), type(students[0]), students)
    names = ""
    for student in students:
        print(student.firstname)
        names += f" {student.firstname}"

    return f"<h1>students: {names}</h1>"


@router.route("/get_string")
def get_string():
    value = 12
    return f"get_string {value}"  # interpolation


@router.route("/get_html_string")
def get_html_string():
    return "<h1>get_html_string</h1>" + "<li>123</li>"  # concatenation


@router.route("/get_json")
def get_json():
    elem1 = 8
...
@router.route("/get_dynamic/<string:ml>")  # != /get_dynamic
def get_dynamic(ml="1"):
    return f"<h1>data: {ml}</h1>"


@router.route("/get_args")
...
@router.route("/get_html_form", methods=["GET", "POST"])
def get_html_form():
    context = {"result": ""}
    if request.method == "POST":
        ...
        datetime_ = request.form.get("datetime", datetime.now().strftime("%Y-%m-%dT%H:%M"))
...
@router.route("/get_data_from_db", methods=["GET", "POST"])
